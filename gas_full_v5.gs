...
function doPost(e){
  ...
  try{
    ...
    if(body.events){
      ...
      else if(action==="toggleExtraSlot")result=toggleExtraSlot(body.date,body.time,body.add);
...
function saveTrafficAccidentConsult(data){
  try{
    ...
    var painDisplay=(data.painAreas||[]).join("、")+(data.painAreasNote?"（"+data.painAreasNote+"）":"");
    var newRow=[
    ...
    //   ここでの失敗は、既に成功している相談内容の保存・LINE通知には影響させない（別途try/catchで保護）
    if(data.hope==="予約したい"){
      try{
        ...
        // Web予約リクエスト一覧への登録に失敗しても、相談自体の受付は成功として扱う
        Logger.log("Web予約リクエスト一覧への自動登録に失敗: "+wErr);
      }
    }
